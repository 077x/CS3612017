 ...
 * 
 * Each method corresponds to a concrete syntax grammar rule, which appears as a comment at the beginning of the method.
 * 
 * This code DOES NOT implement a parser for JAY. You have to complete
 * the code and also make sure it implements a parser for JAY - not something else.
 * 
 *************************************************************************/
...
public class ConcreteSyntax {
	...
	// Match a string with the value of a token. If no problem, go to the next token otherwise generate an error message
	...
	private Conditional ifStatement() {
		...
		c.test = expression();
		match(")");
		c.thenbranch = statement(); // when the statement goes "if(statement)" then...
		token = input.nextToken();
		if(token.getValue().equals("else")) { //check for an "else" statement	
